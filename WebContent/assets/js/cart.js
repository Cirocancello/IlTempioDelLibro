function addToCart(id, quantity, element) {
	...
	$.post(`/iltempiodeldigitale-website/addtocart`, { pId: id, q: quantity }, (data) => {
...
function removeFromCart(id) {
	$.post(`/iltempiodeldigitale-website/addtocart`, { pId: id, q: 0 }, () => {
		location.reload();
	});
}
		...
		if (quantity !== '0') {
			...
			for (let i = 0; i < parsedData.prodotti.length; i++) {
				...
				if (entry.prodotto.idProdotto === id) {
					...
					<form action="/iltempiodeldigitale-website/addtocart" method="POST">
